...
class ImageUtils {  
   ...
   */
  function setFontFile($a) {   $this->fontFile = $a;}
  ...
  function set_line_color($hex) {  $this->line_color = $this->create_color($hex);  }
  function set_thickness($thick = 1) { imagesetthickness ($this->im, $thick);  } 
  ...
  function add_text_align($text, $align, $valign, $margin = 3) {
    ...
    //exit();
    switch ($align) {
    case 'L' : $xoffset = $margin; break;
    case 'R' : $xoffset = $w - $fontwidth - $margin; break;
    case 'M' : $xoffset = $w / 2  - $fontwidth; break;
    default  : $xoffset = $margin;
    }
    switch ($valign) {
    case 'T' : $yoffset = $margin + $fontheight; break;
    case 'D' : $yoffset = $h - $margin; break;
    case 'M' : $yoffset = $h / 2 + $fontheight; break;
    default  : $yoffset = $margin + $fontheight;
    }
  ...
  function dynamic_bg($type, $color = '000000', $plotnost = 0.5, $interval = 10) {    
    ...
    case 20 : case 21 : case 22 : case 23 : case 24 : case 25 : case 26 : case 27 : case 28 : case 29 :
      for ($i = 0; $i < $h; $i += $interval) {
        switch ($type) {
        case 20: imageellipse($this->im,  rand(0,$w), rand(0,$h),  rand(0,$w), rand(0,$h),    $color1); break;
        case 21: imageellipse($this->im,  $w/2 - ($i-round($i/100)*100), $i,  10, 10,    $color1); break;
        case 28: imageellipse($this->im,  rand($w/2 - 10, $w/2 + 10), $i,  10, 10,    $color1); break;
        case 29: imageellipse($this->im,  rand(0,$w), rand(0,$h),  rand(0,$i), rand(0,$i),    $color1); break;
        } 
      }
      break;
